...
# mon test ci dessous:        
        ...
        id=credentials['id']
        key=credentials['key']
#         self.auth = HawkAuth(algorithm, id, key)
#         self.auth = HawkAuth(credentials={'id': id, 'key': key, 'algorithm': 'sha256'})
        self.auth = HawkAuth(id=credentials['id'], key=credentials['key'])
#l original qui plante        
